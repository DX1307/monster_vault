...
function OpenVaultInventoryMenu(data)
		...
		ESX.TriggerServerCallback("monster_vault:getVaultInventory",function(inventory)
				if not inventory then
					  exports['okokNotify']:Alert("Vault", "You don't have a key to the safe!", 5000, 'error')
				else
	...
	while true do
		local sleepthread = 1000
			...
			for k,v in pairs(Config.Vault) do
				...
				if dist < 4 then
					DrawText3Ds(v.coords.x, v.coords.y, v.coords.z +0.5, Config.Text3D)
					sleepthread = 7
