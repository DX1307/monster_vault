...
RegisterNUICallback(
    ...
    -- if Config.disableGive and Config.disablePutIntoVault[data.item.name] then
	if CkeckBlackList(data.item.name, Config["Safe_blacklist"]) then
        exports.PorNotify:SendNotification(
            {
                text = "ไม่สามารถนำของชิ้นนี้ใส่ตู้เซฟได้",
                type = "error",
                timeout = 3000,
                layout = "centerLeft",
                queue = "inventoryhud"
